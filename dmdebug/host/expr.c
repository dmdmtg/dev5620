...
struct expr *e;
{
	...
	if(e->e_lvalue)	printf( "lval=%ld%s ", e->e_lvalue, e->e_hostr?"(host)":"" );
	if( e->e_op )	printf( "op=%c ", e->e_op );
	if( e->e_id )	printf( "id=%s ", e->e_id );
	if( e->e_sym )	printf( "sym=%s ", e->e_sym->n_name );
	if( e->e_reg )	printf( "reg " );
	if( e->e_left )	printf( "left=%ld ", e->e_left );
	if( e->e_right )printf( "right=%ld ", e->e_right );
	if( e->e_mcc ){
		struct ramnl dummy;
		dummy.n_desc = e->e_mcc;
		printf( "%s ", mcctype(&dummy) );
	}
	if( e->e_tyid ) printf( "%s ", e->e_tyid->n_name );
...
struct expr *e;
{
	...
	switch( dotty ){
		case BYTEDOT:
			showdot();
			pokebyte( dot, e->e_rvalue );
			break;
		case SHORTDOT:
			showdot();
			pokeshort( dot, e->e_rvalue );
			break;
		case LONGDOT:
			showdot();
			pokelong( dot, e->e_rvalue );
			break;
		default:
			printf( "use .b, .sh, or .l to set operand length for =\n" );
			return;
	}
	showdot();
}
	...
	case E_BINARY:
		...
		case '-':
			...
			if( ISPTR(left->e_mcc) && numeric(right->e_mcc) ){
			    	size = mccsize(DECREF(left->e_mcc),left->e_tyid ,0);
				if( !size)
				    exprerr(fmt("invalid left side of %c",e->e_op));
				e->e_rvalue = left->e_rvalue +
			...
			if( e->e_op == '+' && 
				...
				size = mccsize( DECREF(right->e_mcc),right->e_tyid ,0);
...
MLONG p;
{
	...
	case 0:
		...
		case SHORT:
		case USHORT:	return (MSHORT) peekshort( r ? p+2 : p );
		case ENUMTY:
		case UNSIGNED:
		case INT:
		case LONG:
...
struct expr *e;
{
	...
	if( e->e_tag != E_ID ) wrap( 1 );
	if( !(s = lookup( USER_MPX, S_BSTR, tyid->n_name, 0 ) ) )
		exprerr( fmt("missing tables for struct %s", tyid->n_name) );
...
struct expr *e;
{
	...
	assert( left->e_tag == E_ID );
	assert( bfun = lookup( USER_MPX, S_GSYM, left->e_id, 0 ) );
	assert( ISFTN( bfun->n_desc ) );
	...
	if( lastpsym )
	    argspace = lastpsym->n_value + mccsize( lastpsym->n_desc, psym+1 ,0);
	argspace /= 4;
	argsave = (MLONG *) talloc( (argspace+1)*4 );
	for( i = 0; i < argspace; ++i ) argsave[i] = peeklong( fp+i*4 );
	for( psym = bfun; psym->n_other != S_EFUN; ++psym )
	    if( psym->n_other == S_PSYM ){
		if( !r ) exprerr( fmt("too few arguments to %s()", left->e_id) );
		assert( r->e_op == ',' );
		if( !eval(r->e_left) ) return 0;
		assignee = enode(E_ID,fmt("%s(%s)",left->e_id,psym->n_name));
	...
	if( e->e_mcc == STRTY || e->e_mcc == UNIONTY ){
		addr_desc( I_STRUCT, DO_INVOKE );
	...
	for( i = 0; i < argspace; ++i ) pokelong( fp+i*4, argsave[i] );
	return 1;
}
...
struct expr *e;
{
	...
	if( ISPTR(left->e_mcc) || left->e_reg )
		...
		case SHORT:
		case USHORT:
			pokeshort( left->e_lvalue, right->e_rvalue );
			break;
		case INT:
		case UNSIGNED:
		case ENUMTY:
...
struct expr *e;
{
	...
	for( ; s->n_other != S_ESTR; ++s ) if( s->n_other == S_SSYM ){
		...
		if( s->n_desc == STRTY ){
			assignee = enode(E_ID,fmt("%s.%s",left->e_id, s->n_name));
