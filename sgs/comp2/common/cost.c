...
static get_leaf(p)
	...
	register struct leaf *lf;
			/*see if this leaf/type pair is in the tables*/
	/* multiple flavored ICON's don't save properly */
	if ( p->tn.op == ICON ) return(0); 
	recost_leaf=0;
...
static save_leaf(p)
	register NODE *p;
	{
			/*save the costs of this leaf for future reference.
			  if recost_leaf is non-zero, it is already in the
			  table*/
	register struct leaf *lf;
	if ( p->tn.op == ICON )
	{
		/* ICONs have too many flavors to save by op */
		recost_leaf=0;
		return;
	}
