#
#	TWO PROCESS BELLMAC-32 COMLD MAKEFILE
#
#	@(#) twoproc.mk: 1.16 12/23/83
#
...
#	$(COMINC)/sdpsrc/lo/writeframe.c $(COMINC)/sdpsrc/hi/allocate.c \
...
#
OBJFL1=expr0.o globs.o sglobs.o ld00.o lex.yy.o lists.o \
	 util.o version.o y.tab.o special1.o
OBJFL2=alloc.o specsyms.o dump.o expr1.o globs.o sglobs.o ld01.o ld1.o ld2.o lists.o maps.o \
	 output.o sdp.o slotvec.o syms.o tv.o util.o version.o \
	 instr.o special2.o
#
#
#
...
# PASS 1
ld:		$(OBJFL1) $(LIBLD)
	$(LD_CMD) -o ld $(OBJFL1) $(LIBES) $(LIBLD) $(LIBS)
#
# PASS 2
ld2:		$(OBJFL2) $(LIBLD)
	$(LD_CMD) -o ld2 $(OBJFL2) $(LIBLD) $(LIBS)
#
#
# Make the *.c:
#
...
ld1.o:		$(COMLD)/extrns.h $(COMLD)/list.h $(COMLD)/ldtv.h \
		$(COMLD)/ldmacros.h $(COMLD)/structs.h system.h \
		$(COMINC)/filehdr.h $(COMINC)/scnhdr.h $(COMINC)/syms.h \
...
sglobs.o:	$(COMLD)/structs.h $(COMINC)/filehdr.h $(COMINC)/scnhdr.h $(COMINC)/syms.h $(COMINC)/storclass.h \
	...
	$(CC_CMD) sglobs.c
#
special1.o:	system.h $(COMLD)/structs.h $(COMLD)/extrns.h \
		special1.c $(COMINC)/filehdr.h $(COMINC)/scnhdr.h $(COMINC)/syms.h \
		$(COMINC)/storclass.h
	$(CC_CMD) special1.c
#
special2.o:	system.h $(COMINC)/aouthdr.h $(COMLD)/structs.h $(COMINC)/filehdr.h \
...
#
#
# Compile the display program, for dumping the ld intermediate file
# used to pass information between PASS 1 and PASS 2
#
display:	globs.o display.o
	$(LD_CMD) -o display display.o globs.o
#
display.o:	system.h $(COMLD)/structs.h $(COMLD)/extrns.h \
		$(COMLD)/ldtv.h \
		$(COMINC)/filehdr.h $(COMINC)/scnhdr.h $(COMINC)/syms.h \
		$(COMINC)/storclass.h \
		$(M32INC)/tv.h \
		y.tab.h \
		$(COMLD)/display.c
	$(CC_CMD) $(COMLD)/display.c
#
#
# File and administrative commands:
#
#
install:	$(BINDIR)/$(SGS)ld $(LIBDIR)/$(SGS)ld2
#
$(BINDIR)/$(SGS)ld:	ld
	-rm -f $(BINDIR)/$(SGS)ld
	cp ld $(BINDIR)/$(SGS)ld
...
$(LIBDIR)/$(SGS)ld2:	ld2
	-rm -f $(LIBDIR)/$(SGS)ld2
	cp ld2 $(LIBDIR)/$(SGS)ld2
	-$(STRIP_CMD) $(LIBDIR)/$(SGS)ld2
#
save:	$(BINDIR)/$(SGS)ld $(LIBDIR)/$(SGS)ld2
	-rm -f $(BINDIR)/$(SGS)ld.back
	-rm -f $(LIBDIR)/$(SGS)ld2.back
	cp $(BINDIR)/$(SGS)ld   $(BINDIR)/$(SGS)ld.back
	cp $(LIBDIR)/$(SGS)ld2  $(LIBDIR)/$(SGS)ld2.back
#
uninstall:	$(BINDIR)/$(SGS)ld.back $(LIBDIR)/$(SGS)ld2.back
	-rm -f $(BINDIR)/$(SGS)ld
	-rm -f $(LIBDIR)/$(SGS)ld2
	cp  $(BINDIR)/$(SGS)ld.back    $(BINDIR)/$(SGS)ld
	cp  $(LIBDIR)/$(SGS)ld2.back   $(LIBDIR)/$(SGS)ld2
#
shrink:
	-rm -f *.o y.tab.c lex.yy.c y.tab.h ld ld2
#
