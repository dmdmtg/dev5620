...
/*									*/
/* */
static char ID[] = "@(#) tv.c: 1.18 3/2/84";
...
/********WARNING---TRANSFER VECTORS NO LONGER WORK FOR 2 PASS LD**********/
		...
		for( sptr = infl->flishead; sptr != NULL; sptr = sptr->isnext ) {
			if( (numreloc = sptr->ishdr.s_nreloc) > 0 ) {
				if (FSEEK(rfd,(long) (FILHSZ+HEADER(rfd).f_opthdr+
					(sptr->isecnum-1) * SCNHSZ),BEGINNING) == -1 ||
			    	FREAD(&shdr,SCNHSZ,1,rfd) != 1 ||
			    	FSEEK(rfd,shdr.s_relptr,BEGINNING) == -1)  {
			...
			while( numreloc-- ) {
				...
				if (FSEEK(ifd,HEADER(ifd).f_symptr+
					rentry.r_symndx * SYMESZ,BEGINNING) == -1 ||
