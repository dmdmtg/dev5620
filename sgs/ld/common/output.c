...
/*									*/
/* */
static char ID[] = "@(#)output.c	1.45 6/6/84";
#include <stdio.h>
 ...
 */
	...
	for( isp = infl->flishead; isp; isp = isp->isnext ) {
		...
		if ((isp->ishdr.s_nreloc == 0)
			...
			copy_section( ifd, infl, isp, fdes, sect_buffer,
				buffer_size );
		else
		{
			if( rflag )
 ...
 */
	...
	if( infl->flsymptr <= 0L ||
	    fseek(infd, infl->flsymptr + infl->flfiloff, BEGINNING) == -1 )  {
		free( newscnum );
		return(0);
		}
	...
	for( count = 0; count < infl->flnsyms; count++)  {
		...
		case C_EXTDEF:
			...
			if( gsp->smlocflg  &&  (secnum > 0) ) {
				...
				if( sm->n_numaux > 0 )  {
					if (fseek( infd, (long) (sm->n_numaux * AUXESZ), 1) == -1)
				...
				if ( (gsp->sment.n_scnum == 0) && aflag &&
			 	...
			 	if( sm->n_numaux > 0 ) {
					if (fseek( infd, (long) (sm->n_numaux * SYMESZ), 1) == -1)
