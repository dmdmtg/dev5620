...
/*									*/
/* */
static char ID[] = "@(#) display.c: 1.2 12/23/83";
 ...
 */
	...
	printf( "\n\t%s%04x %s (%s) %d (%s) %04x (%s)\n\t\t\t%d %08lx %d %d (%04x,%04x)",
		...
		a.dfnscn.aiowname, a.dfnscn.aiattown,
		a.dfnscn.aiblock, (unsigned) a.dfnscn.aifillfg,
		(unsigned) a.dfnscn.aisctype, a.dfnscn.sectspec.head, a.dfnscn.sectspec.tail );

	if (a.dfnscn.aibndadr)
	{
 ...
 */
	...
	case INT:
	case NEXT:
		printf( "\n\t\t\t%s %08lx", OP[(unsigned) e->vnode.exop,
			e->vnode.exvalue );
		break;
	case DOT:
		printf( "\n\t\t\tDOT ()" );
