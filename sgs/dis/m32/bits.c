...
/* */
/*
 *	static	char	bits_ID[] = "@(#)bits.c	1.7 3/16/84";
 */
#include	<stdio.h>
#include	"dis.h"
#include	"filehdr.h"
...
char	*result;
{
	...
	case	ABAD:
		lngtmp = getword(t_ptr);
		longprint(result,(tp->typ==ABAD)? "$": "",lngtmp);
	...
	case	ABADDF:
		lngtmp = getword(t_ptr);
		longprint(result,(tp->typ==ABADDF)? "*$": "*",lngtmp);
		sprintf(result,"%s%s",result,temp2);
		return;
...
long	tmplng;
{
	...
	if (tmplng & 0x00008000L) {
		...
		tmplng = start - neglng;
		(oflag > 0)? sprintf(result,"-0%lo <0%lo>",neglng, tmplng):
