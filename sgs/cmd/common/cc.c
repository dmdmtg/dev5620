...
#ifdef M32
#define	OPTSTR	"cpfOSW:y:t:EPB:go:D:I:U:C1:2:d:VQv:R:l:rsu:xmniL:N#"
#endif
	...
	char *argv[];
{
	...
	av = (char **) stralloc(c + 5 * sizeof(char *));
...
strtcase:	switch (c) {
		...
		case 'f':	/* floating point interpreter */
			...
			fflag++;
#else
			fprintf(stderr, "-f option ignored on 3B\n");
#endif
			break;
#endif
...
#if M32
		case 'y': /* percentage of growth allowence for the optimizer */
 			for (t = optstr; *t && *t!=optopt; t++);
			if (*t && *++t==':')
				i = 1; /* option has an arg */
			else
		...
		case 't':
			...
			case 'v':	/* transfer vectors */
				tvflag++;
				list[Xc0][nlist[Xc0]++] = "-tv";
#ifndef ONEPASS
				list[Xc1][nlist[Xc1]++] =
#endif
				list[Xas][nlist[Xas]++] = "-tv";
...
filearg:		if (((c = getsuf(t)) == 'c') ||
	...
	if (nc && !(Pflag || Eflag)) /* more than just the preprocessor is
