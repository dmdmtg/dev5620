...

{
	...
	if (vldldptr(ldptr) == FAILURE)
	{
		...
		else if ( strncmp( magicbuf, ASCII_MAGIC, AMAGSIZE ) == 0)
		{
			...
			ar_origin = ASCIIAR;
			TYPE(ldptr) = USH_ARTYPE;
			if ((fseek(ioptr, (long)AMAGSIZE, BEGINNING) != -1)
...

{
	...
	if (ldshread(ldptr, sectnum, &shdr) == SUCCESS) {
		...
		if (shdr.s_scnptr != 0) {
		    if (FSEEK(ldptr, shdr.s_scnptr, BEGINNING) != -1) {
			    return(SUCCESS);
		    }
		}
	}
...

{
	...
	if (ldshread(ldptr, sectnum, &shdr) == SUCCESS) {
		...
		if (shdr.s_nreloc != 0) {
		    ...
		    if (FSEEK(ldptr, shdr.s_relptr, BEGINNING) != -1) {
			    return(SUCCESS);
		    }
		}
	}
...

{
	...
	if (ar_origin == RANDOM)
	{
		...
		if ((FSEEK( ldptr, -((long) sizeof(RANDARF)), BEGINNING) != -1) &&
	...
	else if (ar_origin == ASCIIAR)
	{
		...
		if ((FSEEK( ldptr, -((long) sizeof( PORT_HEAD )), BEGINNING) != -1)
		...
		if ((FSEEK(ldptr, -((long)asize), BEGINNING) == -1)
