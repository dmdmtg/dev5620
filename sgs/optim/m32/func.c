...
int nodblflg = 0;	/* flag indicating presence of doubles */
...

{
	...
	/* set doubles flag */
	case 'N': nodblflg = 1; return; /* there are dubbles */
	}
...
int newnreg;	/* new number of registers to be saved/restored */
{
	...
	for( pa = pamin; pa <= pacut; pa++ ) {
		if( pa->asvname && pa->asrname &&
				strcmp( pa->asvname, pa->asrname ) == 0 )
			pa->asvscl = SCLNULL;
	}
...
extern char *mktemp();
char	itmpname[256]; /* name of file for output before in-line subst */
