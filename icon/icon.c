...
Rectangle r;
{
	...
	for (j = 0; j < yicons; j++) {
		for (i = 0; i < xicons; i++) {
			texture16(textr, textr->rect, imenu[j*(xicons+1)+i], F_STORE);
...
char *chstring;
{
	string(&defont, chstring, &display, add(icon.corner, Pt(15, -20)), F_XOR);
	sleep(120);
	string(&defont, chstring, &display, add(icon.corner, Pt(15, -20)), F_XOR);
}
...
/***********************************************************************
    This routine is a library routine that is not included by TELETYPE,
so I've included the host one here.
***********************************************************************/
char *
strrchr(sp, c)
register char *sp, c;
...
char	*str;
{
	texture16(b, raddp(rect16x16, Pt(16, 16 * i)), icon, F_XOR);
	string(&defont, str, b, Pt(44, 16 * i), F_XOR);
}


HelpSorry()
{
	string(&defont, "Not enough space on dmd",
...
char	*argv[];
{
	...
	strcpy(dmdpath, ICONDIR);	/* default value */
